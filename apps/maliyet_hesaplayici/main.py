...
def hesapla():
    try:
        ...
        for veri in proje_verileri:
            ...
            if veri["tip"] == "ISCILIK": 
                ham_i += tutar
                satis_i += tutar * (1 + (marj_iscilik / 100))
            else: 
                ham_m += tutar
                satis_m += tutar * (1 + (marj_malzeme / 100))
        
        ham_toplam_usd = ham_m + ham_i
...
def excele_aktar():
    ...
    try:
        ...
        if messagebox.askyesno("Excel Kaydedildi", f"Konum: {tam_yol}\nKlasörü açmak ister misiniz?"): os.startfile(hedef_klasor)
...
def pdf_olustur_ve_ac():
    ...
    pdf_yol = os.path.join(hedef_klasor, dosya_adi)
    ...
    try:
        ...
        try: 
            pdfmetrics.registerFont(TTFont('Arial', 'arial.ttf')); pdfmetrics.registerFont(TTFont('Arial-Bold', 'arialbd.ttf'))
            font_n='Arial'; font_b='Arial-Bold'
        ...
        c.save()
        if messagebox.askyesno("PDF Hazır", f"Konum: {pdf_yol}\nAçmak ister misiniz?"): os.startfile(pdf_yol)
    except Exception as e: messagebox.showerror("Hata", str(e))
...
def manuel_mod_kontrol():
    if var_manuel.get() == 1: 
        ...
        cmb_kategori.configure(state="readonly") 
        secilen = cmb_kategori.get()
...
def sifirla():
    if messagebox.askyesno("Sıfırla", "Tüm liste silinecek?"):
        ...
        for lbl in [lbl_ham_toplam_val, lbl_satis_toplam_val, lbl_tl_kdvli_val]:
            lbl.configure(text="...")
